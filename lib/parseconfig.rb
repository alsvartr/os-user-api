...
class ParseConfig
  ...
  attr_accessor :config_file, :params, :groups, :cli, :merged
  ...
  def initialize()
    ...
    @groups = []
    self.cli = Hash.new()
    self.merged = Hash.new()
  end

   def parse_all(config_file=nil)
      self.parse_file(config_file) if config_file != nil
      self.parse_cli()
      self.merge()
   end
   ...
   def parse_cli()
         params = ARGV
         return nil if params.size == 0

         params.each do |param|
            param = param.split("=")
            var = param[0][/^-{1,2}([a-z]+|[0-9]+)/]
            val = param[1]
            next if var == nil
   ...
   def merge()
      self.merged = self.params.merge(self.cli)
   end
   ...
   def types!()
      self.merged.each do |var, val|
         self.merged[var] = self.types(val)
      end

      self.params.each do |var, val|
	 self.params[var] = self.types(val)
      end
   end
